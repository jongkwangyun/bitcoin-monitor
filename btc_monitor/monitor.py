...
from btc_monitor.report_manager import format_regular_report, should_send_regular_report
from btc_monitor.signal_manager import check_ma_cross, load_alert_cache, save_alert_cache
...
def monitor_cross_signal(s, df_live, market, token, chat_id, cache_path):
    ...
    if msg_120:
        alerts_to_send.append(msg_120)

    msg_200 = check_ma_cross(cache, "200", ma200, live_price)
    if msg_200:
        alerts_to_send.append(msg_200)
...
def main() -> int:
    ...
    cache_path = os.environ.get("BTC_ALERT_CACHE", str(PROJECT_ROOT / ".btc_alert_cache.json")).strip()
    report_state_path = os.environ.get("BTC_REPORT_CACHE", str(PROJECT_ROOT / ".btc_report_state.json")).strip()
